...
const MapSection = () => {
  ...
  return (
    <div className="bg-white rounded-[24px] xl:rounded-[40px] p-4 md:p-6 shadow-sm border border-gray-50 relative flex flex-col h-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 md:mb-6 relative z-[1000]">
        <div className="flex-shrink-0">
          <h3 className="text-lg md:text-xl xl:text-2xl font-black text-[#1B2559] tracking-tight">კამპუსების რუკა</h3>
          <p className="text-[10px] md:text-xs text-slate-400 font-medium">იპოვე შენი ლოკაცია</p>
        </div>
        ...
        <div className="relative w-full sm:w-56 xl:w-72">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] xl:text-xs font-bold text-[#1B2559] uppercase tracking-tighter">სწრაფი ძიება...</span>
            </div>
            ...
            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-2xl border border-gray-50 overflow-hidden z-[1001] animate-in fade-in slide-in-from-top-1 duration-200">
              <div className="p-1 max-h-40 xl:max-h-60 overflow-y-auto custom-scrollbar">
                {universities.map((uni) => (
                  ...
                  >
                    <div className="w-7 h-7 bg-white rounded-md p-0.5 shadow-sm flex-shrink-0">
                      <img src={uni.image} alt="" className="w-full h-full object-contain" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-[9px] xl:text-[10px] font-black text-[#1B2559] uppercase tracking-tighter truncate">{uni.name}</p>
                      <p className="text-[7px] xl:text-[8px] text-slate-400 font-bold truncate">{uni.address}</p>
                    </div>
                  </button>
                ))}
              </div>
      ...
      <div className="h-[280px] sm:h-[320px] lg:h-[380px] xl:h-[450px] w-full rounded-xl xl:rounded-[32px] overflow-hidden border-2 xl:border-4 border-white shadow-xl relative z-[10]">
        <MapContainer 
              ...
              <Popup className="custom-popup">
                <div className="p-0 min-w-[160px]">
                  <div className="flex items-center gap-2 mb-2">
                    <img src={uni.image} className="w-6 h-6 rounded-md object-contain bg-slate-50 p-1" />
                    <h4 className="font-black text-[#1B2559] text-[10px] uppercase tracking-tighter leading-tight">{uni.shortName}</h4>
                  </div>
                  <div className="space-y-1 mb-2">
                    <div className="flex items-center gap-1.5 text-slate-500">
                        <MapPin size={8} className="text-brand-primary" />
                        <span className="text-[8px] font-bold leading-tight">{uni.address}</span>
                    </div>
                  </div>
      ...
      <style jsx global>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #4318FF; border-radius: 10px; }
